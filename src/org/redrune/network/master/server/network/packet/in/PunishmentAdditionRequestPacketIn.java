...
import org.redrune.network.master.server.network.packet.out.PunishmentAdditionAttemptPacketOut;
 ...
 * @since 8/17/2017
 */
@Readable(packetIds = { PacketConstants.PUNISHMENT_ADDITION_REQUEST_PACKET_ID })
public class PunishmentAdditionRequestPacketIn implements ReadablePacket<MSSession> {
	...
	public void read(MSSession session, IncomingPacket packet) {
		...
		Arrays.stream(MSRepository.getWorlds()).filter(Objects::nonNull).forEach(world -> world.getSession().write(new PunishmentAdditionAttemptPacketOut(punishment)));
	}
}
