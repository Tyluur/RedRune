...
import org.redrune.network.master.server.network.packet.out.PunishmentRemovalSuccessDeliveryPacketOut;
 ...
 * @since 8/17/2017
 */
@Readable(packetIds = { PacketConstants.PUNISHMENT_REMOVAL_SUCCESS_ALERT_PACKET_ID })
public class PunishmentRemovalSuccessPacketIn implements ReadablePacket<MSSession> {
	...
	public void read(MSSession session, IncomingPacket packet) {
		...
		MSRepository.getSessionByUsername(punisher).ifPresent(worldSession -> {
			worldSession.write(new PunishmentRemovalSuccessDeliveryPacketOut(worldId, punishment, message));
		});
	}
}
